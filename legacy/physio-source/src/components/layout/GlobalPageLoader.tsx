...
const CRITICAL_UI_WAIT_MS = 2500
const HEADER_READY_ATTR = "data-header-ready"
const FOOTER_READY_ATTR = "data-footer-ready"

function isCriticalUiReady() {
  if (typeof document === "undefined") return false
  const root = document.documentElement
  return root.getAttribute(HEADER_READY_ATTR) === "1" && root.getAttribute(FOOTER_READY_ATTR) === "1"
}
      ...
      const onLoaded = async () => {
        ...
        window.setTimeout(() => {
          ...
          const complete = () => {
            if (completed) return
            completed = true
            window.removeEventListener("pk:header-ready", onReady)
            window.removeEventListener("pk:footer-ready", onReady)
            settleRaf()
          }
          const onReady = () => {
            if (isCriticalUiReady()) {
          ...
          const timeout = window.setTimeout(complete, CRITICAL_UI_WAIT_MS)
          window.addEventListener("pk:header-ready", onReady)
          window.addEventListener("pk:footer-ready", onReady)
