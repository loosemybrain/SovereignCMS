...
export function PopupEditorClient({ popupId }: Props) {
  ...
  const handleSave = async () => {
    ...
    // Clientseitige Validierung: Popup kann nur aktiviert werden, wenn "alle Seiten" oder mindestens eine Seite ausgewählt
    if (popup.isActive && !popup.allPages && popup.selectedPageIds.length === 0) {
      alert(
        "Popup kann nicht aktiviert werden: Wählen Sie entweder 'Auf allen Seiten' oder mindestens eine Seite aus. " +
          "Sonst wird das Popup nicht in der öffentlichen Ansicht erscheinen."
      )
      return
    }
            ...
            <CardContent className="space-y-4">
              ...
              {!popup.isActive && (
                <div className="rounded-md bg-blue-50 p-3 text-sm">
                  <div className="font-medium text-blue-900">Entwurf gespeichert</div>
                  <div className="text-xs text-blue-800 mt-1">
                    Dieses Popup ist als Entwurf gespeichert. Aktivieren Sie es erst, wenn entweder "Auf allen Seiten" aktiv ist oder mindestens eine Seite ausgewählt wurde.
                  </div>
                </div>
              )}
