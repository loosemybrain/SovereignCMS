...
import { ColorGroupAccordion, ColorGroup } from "../inspector/ColorGroup"
...
/**
 * Gruppiert Farb-Felder automatisch basierend auf ihren Namen.
 * Ordnet sie in logische Gruppen wie "Grundfarben", "CTA / Buttons", etc.
 */
function groupColorFieldsByName(fields: InspectorField[]): Record<string, InspectorField[]> {
  const groups: Record<string, InspectorField[]> = {}

  // Standardgruppen mit Keywords für intelligente Zuordnung
  const groupMapping: Record<string, string[]> = {
    "Grundfarben": ["background", "text", "headline", "subheadline", "description", "content", "base"],
    "CTA / Buttons": ["cta", "button", "secondary", "action"],
    "Badge / Label": ["badge", "label", "eyebrow", "tag"],
    "Karten / Panels": ["card", "panel", "item", "box"],
    "Rahmen / Trenner": ["border", "divider", "line", "separator"],
    "Dekorative Elemente": ["icon", "accent", "highlight", "dot", "floating"],
    "Formular": ["input", "form", "error", "success", "field", "placeholder"],
  }

  // Gruppiere Felder basierend auf Keywords in ihrem Namen
  fields.forEach((field) => {
    let assigned = false
    const fieldKeyLower = field.key.toLowerCase()

    for (const [groupName, keywords] of Object.entries(groupMapping)) {
      if (keywords.some((kw) => fieldKeyLower.includes(kw))) {
    ...
    // Fallback: "Sonstige" für unklare Farben
    if (!assigned) {
      if (!groups["Sonstige"]) groups["Sonstige"] = []
      groups["Sonstige"].push(field)
    }
  })
  ...
  function renderGroupedInspectorFields(): React.ReactNode {
    ...
    return (
      <React.Fragment>
        ...
        {/* Special-case repeater – ContactForm fields/contactInfoCards with Accordion */}
        {selectedBlock.type === "contactForm" && (
          <>
            <Separator />
            {/* Formular-Felder */}
            {(() => {
              const props = selectedBlock.props as Record<string, unknown>
              const fields = (getByPath(props, "fields") as Record<string, unknown>[]) || []
              const itemFields = [
              ...
              return (
                <div className="space-y-2">
                  <div className="flex items-center justify-between mb-2">
                    <Label className="text-sm font-semibold">Formular-Felder</Label>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => editorActions.handleAddArrayItem(selectedBlock.id, "fields", () => createContactFormField("subject"))}
                      className="gap-1"
                    >
                      <Plus className="h-3 w-3" />
                      Feld hinzufügen
                  ...
                  {fields.length === 0 ? (
                    <p className="text-xs text-muted-foreground py-2">Keine Felder vorhanden</p>
                  ) : (
                    <Accordion type="single" collapsible className="w-full space-y-1.5">
                      {fields.map((field, index) => {
                        ...
                        const fieldId = (f.id as string) || `field-${index}`

                        return (
                          <AccordionItem key={fieldId} value={fieldId} className="rounded-lg border border-border bg-card overflow-hidden">
                            <div className="flex items-center justify-between">
                              <AccordionTrigger className="hover:no-underline flex-1 py-2 px-3">
                                <span className="text-xs font-medium text-left">
                                  {index + 1}. {fieldLabel || fieldType || "Feld"}
                                </span>
                              </AccordionTrigger>
                              <div className="flex items-center gap-0.5 pr-2">
                                <Button
                                  ...
                                  size="icon"
                                  className="h-6 w-6"
                                  onClick={() => editorActions.handleMoveArrayItem(selectedBlock.id, "fields", index, index - 1)}
                                  disabled={index === 0}
                                  title="Nach oben"
                                >
                                  <ChevronUp className="h-3 w-3" />
                                ...
                                <Button
                                  ...
                                  onClick={() => confirmDeleteItem(selectedBlock.id, "fields", index)}
                                  title="Löschen"
                                >
                                  <Trash2 className="h-3 w-3" />
                                </Button>
                            ...
                            <AccordionContent className="space-y-3 border-t border-border p-3 bg-muted/20">
                              {renderOneRepeaterItemFields(selectedBlock, "fields", index, field as Record<string, unknown>, itemFields)}
                            </AccordionContent>
                          </AccordionItem>
                        )
                      })}
            ...
            {(() => {
              const props = selectedBlock.props as Record<string, unknown>
              const cards = (getByPath(props, "contactInfoCards") as Record<string, unknown>[]) || []
              const itemFields = [
              ...
              return (
                <div className="space-y-2 mt-6">
                  <div className="flex items-center justify-between mb-2">
                    <Label className="text-sm font-semibold">Info-Cards</Label>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => editorActions.handleAddArrayItem(selectedBlock.id, "contactInfoCards", createContactInfoCard)}
                      className="gap-1"
                    >
                      <Plus className="h-3 w-3" />
                      Card hinzufügen
                  ...
                  {cards.length === 0 ? (
                    <p className="text-xs text-muted-foreground py-2">Keine Cards vorhanden</p>
                  ) : (
                    <Accordion type="single" collapsible className="w-full space-y-1.5">
                      {cards.map((card, index) => {
                        const c = card as Record<string, unknown>
                        const cardTitle = String(c.title || "")
                        const cardId = (c.id as string) || `card-${index}`

                        return (
                          <AccordionItem key={cardId} value={cardId} className="rounded-lg border border-border bg-card overflow-hidden">
                            <div className="flex items-center justify-between">
                              <AccordionTrigger className="hover:no-underline flex-1 py-2 px-3">
                                <span className="text-xs font-medium text-left">
                                  {index + 1}. {cardTitle || "Card"}
                                </span>
                              </AccordionTrigger>
                              <div className="flex items-center gap-0.5 pr-2">
                                <Button
                                  ...
                                  size="icon"
                                  className="h-6 w-6"
                                  onClick={() => editorActions.handleMoveArrayItem(selectedBlock.id, "contactInfoCards", index, index - 1)}
                                  disabled={index === 0}
                                  title="Nach oben"
                                >
                                  <ChevronUp className="h-3 w-3" />
                                ...
                                <Button
                                  ...
                                  size="icon"
                                  className="h-6 w-6 text-destructive hover:text-destructive"
                                  onClick={() => confirmDeleteItem(selectedBlock.id, "contactInfoCards", index)}
                                  title="Löschen"
                                >
                                  <Trash2 className="h-3 w-3" />
                                </Button>
                            ...
                            <AccordionContent className="space-y-3 border-t border-border p-3 bg-muted/20">
                              {renderOneRepeaterItemFields(selectedBlock, "contactInfoCards", index, card as Record<string, unknown>, itemFields)}
                            </AccordionContent>
                          </AccordionItem>
                        )
                      })}
                    ...
                    <div className="mt-3 pt-3 border-t border-border/50">
                      ...
                      <div className="space-y-3 mt-2">
                        ...
                        {colorFields.length > 0 && (
                          <ColorGroupAccordion>
                            {Object.entries(groupColorFieldsByName(colorFields)).map(([groupName, fields]) => (
                              <ColorGroup 
                                key={groupName}
                                title={groupName}
                              >
                ...
                <>
                  ...
                  <div className="space-y-1.5 rounded-lg bg-muted/30 p-3">
                    ...
                    <ColorGroupAccordion>
                      ...
                      {/* Gruppe 4: Transparenz */}
                      <ColorGroup title="Transparenz">
                        <div className="space-y-1.5">
                        ...
                        <TabsContent value="physiotherapy" className="space-y-4 mt-4">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="rounded-md border border-border bg-muted/20 p-5 space-y-4">
                              <div className="text-xs font-semibold mb-2">Farben</div>
                              <ColorGroupAccordion>
                                {/* Gruppe 1: Grundfarben */}
                                <ColorGroup title="Grundfarben">
                                  <div className="space-y-1.5">
                                  ...
                                  <div className="space-y-1.5">
                                    ...
                                    {(() => {
                                      ...
                                      return (
                                        <Select
                                          value={preset}
                                          onValueChange={(v) => handleBrandFieldChange("badgeRadiusPreset", v)}
                                        >
                                          <SelectTrigger className="h-8 text-sm">
                                ...
                                {/* Gruppe 4: Play Button */}
                                <ColorGroup title="Play Button">
                                  <div className="space-y-1.5">
                                ...
                                {/* Gruppe 5: Trust / Floating Elemente */}
                                <ColorGroup title="Trust / Floating Elemente">
                                  <div className="space-y-1.5">
